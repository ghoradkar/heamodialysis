...
import 'package:heamodialysis/login/get_captcha_model.dart';
...
class LoginController extends GetxController {
      ...
      if (data['status'] == 'Success') {
        ...
        if (loginRespModel != null && unitName != null) {
          var unitId = checkAndExtractUnitId(
            loginRespModel!.data![0].unitId!.toString(),
            unitName!.unitId.toString(),
          );
          loginRespModel!.data![0].unitId = int.parse(unitId);
        }

        await SharedPref().save(
            const SharedPrefConstant().kUserData, loginRespModel?.data![0]);
        status = data['status'];
      } else {
        isLoading = false;
  ...
  Future<bool> getUnitId(String userName) async {
    final uri = Uri.parse(
    ...
    if (response.statusCode == 200) {
      ...
      unitNameList = data.map((item) => UnitNameModel.fromJson(item)).toList();
