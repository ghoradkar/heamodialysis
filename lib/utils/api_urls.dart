class ApiConstants {
  ///Replica
  // static String ipPort = "http://210.89.42.115:9999";

  ///test
  static String ipPort = "http://210.89.42.122:8080";

  ///dev
  // static String ipPort = "http://210.89.42.115:8080";

  static String ip = "$ipPort/$commonPath1/";
  static String commonPath1 = "Hemodialysis-Apis";
  static String commonPath2 = "/cluster-dashborad/";


  static String oldBaseUrl = ipPort.contains('http://210.89.42.122:8080')
      ? "$ipPort/HAEMODIALYSIS/restApi"
      : "$ipPort/HAEMODIALYSIS-Replica/restApi";

  static String baseUrl = "$ipPort/$commonPath1/api/mobile";
  static String baseUrl1 = "$ipPort/$commonPath1/hemodialysis-api";

  static String baseUrl4 = "$ipPort/$commonPath1/dashborad";
  // static String baseUrl5 = "$ipPort/$commonPath3";

  static String baseUrlMIS = "$ipPort/$commonPath1/misDashboard";
  static String baseUrlCluster = "$ipPort/$commonPath1/cluster-dashborad";
  static String baseUrlNeph = "$ipPort/$commonPath1/nephroDashboard";

  static String imageBaseUrl = "$ipPort/HAEMODIALYSIS/Images/";
  static String imageBaseUrl1 = "$ipPort/HAEMODIALYSIS/Uploaded_RO/";

  ///Production
  // static String ipPort =
  //     "https://api.mahadialysis.in/Mahadialysis-Apis/api/mobile";
  // static String ip = "https://api.mahadialysis.in/Mahadialysis-Apis/";
  // static String ip1 = "https://app.mahadialysis.in";
  // static String commonPath1 = "Hemodialysis-Apis";
  // // static String commonPath3 = "Mahadialysis";
  //
  // static String oldBaseUrl = "$ip1/Mahadialysis/restApi";
  //
  // static String baseUrl = ipPort;
  // static String baseUrl1 = "$ip/hemodialysis-api";
  //
  // static String baseUrl4 = "$ip/dashborad";
  // // static String baseUrl5 = "http://103.104.73.130:8080/$commonPath3";
  //
  // static String baseUrlNeph = "$ip/nephroDashboard";
  //
  // static String baseUrl6 = "$ip1/Mahadialysis/restApi";
  // static String baseUrlCluster = "$ip/cluster-dashborad/";
  //
  // static String imageBaseUrl = "$ip1/Mahadialysis/Images/";
  // static String imageBaseUrl1 = "$ipPort/HAEMODIALYSIS/Uploaded_RO/";
  // static String baseUrlMIS = "$ip/misDashboard";
}

// enum Environment { dev, prod, test, replica }
//
// class ApiConstants {
//   ApiConstants._();
//
//   static late Environment _env;
//   static void configure(Environment env) => _env = env;
//
//   // Add this getter so we can restore env in self-checks
//   static Environment get currentEnv => _env;
//
//   // -------- Hosts --------
//   static String get ipPort {
//     switch (_env) {
//       case Environment.prod:
//         return 'https://api.mahadialysis.in';
//       case Environment.test:
//         return 'http://210.89.42.122:8080';
//       case Environment.replica:
//         return 'http://210.89.42.115:9999';
//       case Environment.dev:
//         return 'http://210.89.42.115:8080';
//     }
//   }
//
//   // Secondary host for legacy REST & images in PROD
//   static String get ip1 =>
//       _env == Environment.prod ? 'https://app.mahadialysis.in' : ipPort;
//
//   // API family name: Mahadialysis in PROD, Hemodialysis elsewhere
//   static String get _apiRoot =>
//       _env == Environment.prod ? 'Mahadialysis-Apis' : 'Hemodialysis-Apis';
//
//   // Old REST family bucket
//   static String get _oldRestFamily {
//     if (_env == Environment.replica) return 'HAEMODIALYSIS-Replica';
//     if (_env == Environment.prod) return 'Mahadialysis';
//     return 'HAEMODIALYSIS';
//   }
//
//   // Convenience path with trailing slash
//   static String get ip => '$ipPort/$_apiRoot/';
//
//   // -------- Current APIs --------
//   //  PROD: https://api.mahadialysis.in/Mahadialysis-Apis/api/mobile
//   // Others: .../Hemodialysis-Apis/api/mobile
//   static String get baseUrl => '$ipPort/$_apiRoot/api/mobile';
//
//   static String get baseUrl1 => '$ipPort/$_apiRoot/hemodialysis-api';
//   static String get baseUrl4 => '$ipPort/$_apiRoot/dashborad';
//   static String get baseUrlMIS => '$ipPort/$_apiRoot/misDashboard';
//   static String get baseUrlCluster => '$ipPort/$_apiRoot/cluster-dashborad';
//   static String get baseUrlNeph => '$ipPort/$_apiRoot/nephroDashboard';
//
//   // -------- Legacy REST --------
//   // PROD: https://app.mahadialysis.in/Mahadialysis/restApi
//   // Replica: .../HAEMODIALYSIS-Replica/restApi
//   // Test/Dev: .../HAEMODIALYSIS/restApi
//   static String get oldBaseUrl => _env == Environment.prod
//       ? '$ip1/Mahadialysis/restApi'
//       : '$ipPort/$_oldRestFamily/restApi';
//
//   // -------- Images --------
//   // PROD served from app.mahadialysis.in
//   static String get imageBaseUrl => _env == Environment.prod
//       ? '$ip1/Mahadialysis/Images/'
//       : '$ipPort/HAEMODIALYSIS/Images/';
//
//   static String get imageBaseUrl1 => '$ipPort/HAEMODIALYSIS/Uploaded_RO/';
// }
