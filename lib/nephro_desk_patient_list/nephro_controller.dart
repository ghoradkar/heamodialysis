...
// "com.mahadialysis.technician" if this package name is present in the file path means new file is selected or to edit file user selected new file
      ...
      if (response.statusCode == 200) {
        ...
        // if (resp.contains("document uploaded successfully")) {
        // Show success popup
  ...
  Future<void> getNephroList(String type, String input, String unitId,
      String districtId, String? status) async {
    isLoading = true;
    update();
        ...
        checkBoxListClinicalHistory = List.generate(
            ...
            (index) =>
                clinicalHistoryTableData!
                    .listCliniComorBean?[index].comorFlag ==
                "Y");

      // Populate selectedItemsPerRow with matched relations
      for (int i = 0; i < clinicalHistoryTableData!.listCliniComorBean!.length; i++) {
        ...
        List<int>? multiRelaIds = clinicalHistoryTableData!.listCliniComorBean![i].multiRelaId;

        if (multiRelaIds != null && multiRelaIds.isNotEmpty) {
          for (int j = 0; j < multiRelaIds.length; j++) {
            try {
              // Find the relation name by matching ID
              String? relationName = relationList?.firstWhere(
                      (e) => e.lookupDetId == multiRelaIds[j],
              ...
              debugPrint('Error finding relation for ID ${multiRelaIds[j]}: $e');
            }
          }
        }
  ...
  saveInstructions(body, treatmentId, patientId) async {
    isLoading = true;
    update();
    // final uri =
    //     Uri.parse(ApiConstants.baseUrl4 + ApiConstants.getCentralDashboarCount);
  ...
  saveIndivisualInstructions(body, treatmentId, patientId) async {
    isLoading = true;
    update();
    // final uri =
    //     Uri.parse(ApiConstants.baseUrl4 + ApiConstants.getCentralDashboarCount);
